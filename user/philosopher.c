...
void main_philosopher() {
  ...
  for (int i = 0; i < 16; i++) {
    if (fork() == 0) { // Section that controls all the fork children
      ...
      char c[2];
      itoa(c, philosopher_id);

      while( 1 ) {

        // Philosopher thinks
        write( STDOUT_FILENO, "Philosopher ", 12 );write( STDOUT_FILENO, c, 2 );write( STDOUT_FILENO, " is thinking \n", 14 );
        sleep();
        sem_wait(&mutex);
        ...
        if ( spork[philosopher_id] && spork[(philosopher_id + 1) % 16] ) {
          ...
          // Philosopher eats
          write( STDOUT_FILENO, "Philosopher ", 12 );write( STDOUT_FILENO, c, 2 );write( STDOUT_FILENO, " is eating \n", 12 );
          sleep();

          write( STDOUT_FILENO, "Philosopher ", 12 );write( STDOUT_FILENO, c, 2 );write( STDOUT_FILENO, " puts his cutlery down \n", 24 );
